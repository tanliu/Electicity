...
-->
	...
	var zNodes =[
         {id:0, pId:0, name:"北京理工大学珠海学院","target":"add", open:true},
	...
	function onClick(event, treeId, treeNode, clickFlag) {
		window.parent.add.location.href="${basePath}system/organize_addUI.action?organize.parentId="+treeNode.id;
		window.parent.editor.location.href="${basePath}system/organize_editorUI.action?organize.orgId="+treeNode.id;
   ...
   function deleteNode(id){
	...
	$(document).ready(function(){  //异步加载
		...
		$.ajax({
			url:"${basePath}system/organize_treeData.action",
			type:"post",
	    	...
	    	success: function(data){
	    		...
	    		for(var i=0;i<dataObj.length;i++){
		    		var val = {id:dataObj[i].orgId, pId:dataObj[i].parentId, name:dataObj[i].orgName, open:true};
		    		zNodes.push(val); 
	    		}
	    		}
	    		//加载树
